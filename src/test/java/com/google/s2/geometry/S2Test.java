...
public strictfp class S2Test extends BaseTest {
  ...
  public void testSTUV() {
    ...
    for (double x = -1; x <= 1; ++x) {
      assertEquals(S2Projections.stToUV(x), x, 0.01);
      assertEquals(S2Projections.uvToST(x), x, 0.01);
    }
  ...
  public void testFaceUVtoXYZ() {
    ...
    for (int face = 0; face < 6; ++face) {
      ...
      assertEquals(Math.abs(center.get(center.largestAbsComponent())), 1.0, 0.01);
    ...
    for (int face = 0; face < 6; ++face) {
      assertEquals(
          S2Point.crossProd(S2Projections.getUAxis(face), S2Projections.getVAxis(face)).dotProd(
              S2Projections.faceUvToXyz(face, 0, 0)), 1.0, 0.01);
    }
  ...
  public void testAngleArea() {
    ...
    assertEquals(S2.area(pr, pr, pr), 0.0, 0.01);
    ...
    assertEquals(S2.area(p000, p045, p090), 0.0, 0.01);
