...
public final strictfp class S2Polyline implements S2Region {
  ...
  public int getSpaceDistance(S2Point s2Point){
    Preconditions.checkState(numVertices() > 0, "Empty polyline");
    ...
    int minIndex = -1;
    S2LatLng targetPoint = new S2LatLng(s2Point);
    for (int i = 0; i <numVertices()-1 ; i++) {
      double earthDistance = targetPoint.getEarthDistance(new S2LatLng(vertex(i)));
